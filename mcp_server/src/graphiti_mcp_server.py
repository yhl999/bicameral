...
def _build_get_tools_response() -> list[dict[str, Any]]:
    return [
        ...
        _tool_schema_entry(
            name='remember_fact',
            description='Write a typed fact to the memory ledger or quarantine a conflicting candidate for review',
            mode_hint='typed',
            inputs={
                'text': 'string — natural-language or structured fact',
                'hint': 'object | null — extraction hints (fact_type, subject, scope, supersede, etc.)',
            },
        ...
        _tool_schema_entry(
            name='get_history',
            description='Retrieve ledger event summaries for the currently active root(s) matching a subject/predicate',
            mode_hint='typed',
            inputs={
                'subject': 'string',
                'predicate': 'string | null',
            ...
            output='dict with history: list[ChangeEventSummary], roots_considered: list[string] | ErrorResponse',
            examples=[{'subject': 'project-alpha', 'predicate': 'status'}],
        ),
        _tool_schema_entry(
            name='list_candidates',
            description='List remember_fact conflict candidates currently quarantined for review',
            mode_hint='typed',
            inputs={'status': '"pending" | "promoted" | "rejected" | null'},
            output='dict with candidates: list[Candidate] | ErrorResponse',
            examples=[{'status': 'pending'}],
        ),
        _tool_schema_entry(
            name='promote_candidate',
            description='Resolve a pending conflict candidate by promoting it with resolution="supersede" (or canceling it)',
            mode_hint='typed',
            inputs={'candidate_id': 'string', 'resolution': '"supersede" | "cancel"'},
            output='dict with candidate/fact/promotion metadata | ErrorResponse',
            examples=[{'candidate_id': 'cand-001', 'resolution': 'supersede'}],
        ),
        _tool_schema_entry(
            name='reject_candidate',
            description='Reject a pending candidate and keep the current fact unchanged',
            mode_hint='typed',

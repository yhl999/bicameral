...
def _build_get_tools_response() -> list[dict[str, Any]]:
    return [
        ...
        _tool_schema_entry(
            ...
            description='List quarantined fact candidates awaiting review or historical audit',
            mode_hint='typed',
            inputs={
                'status': '"quarantine" | "promoted" | "rejected" | null',
            ...
            examples=[{'status': 'quarantine', 'age_days': 7}],
            phase0_behavior='Returns persisted candidate rows that match the filters.',
        ),
        _tool_schema_entry(
            name='promote_candidate',
            description='Promote a quarantined candidate into the typed ledger with conflict resolution',
            mode_hint='typed',
            inputs={
                'candidate_id': 'string',
            ...
            output='{"candidate": Candidate, "promotion": object} | {"candidate": Candidate, "action": "cancelled"} | ErrorResponse',
            examples=[{'candidate_id': 'cand-001', 'resolution': 'supersede'}],
            phase0_behavior='Performs real promotion/cancel operations against candidates + change_ledger tables.',
        ),
        _tool_schema_entry(
            name='reject_candidate',
            description='Reject a quarantined candidate without promoting it to the ledger',
            mode_hint='typed',
            inputs={'candidate_id': 'string', 'reason': 'string | null'},
            output='{"candidate": Candidate, "action": "rejected"} | ErrorResponse',
            examples=[{'candidate_id': 'cand-002'}],
            phase0_behavior='Performs a persisted candidate-status transition to rejected.',
        ),
        _tool_schema_entry(
            name='list_packs',

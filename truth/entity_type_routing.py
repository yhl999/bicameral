...
def route_entity_edge(
    *,
    ...
    if all_labels & PROCEDURE_ENTITY_LABELS:
        ...
        # Source wins in the both-labeled case: prefer a_set first so that
        # when both sides are labeled, source_name is chosen.
        typed_name = source_name if PROCEDURE_ENTITY_LABELS & a_set else target_name
        return (
            "procedure.steps",
            "procedure",
            {
    ...
    if all_labels & EPISODE_ENTITY_LABELS:
        # Same side-selection logic: source wins when both sides are labeled.
        typed_name = source_name if EPISODE_ENTITY_LABELS & a_set else target_name
        return (
            "episode.description",
            "episode",
            {

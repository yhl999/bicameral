...
def _label_sets_compatible(left: frozenset[str], right: frozenset[str]) -> bool:
...
def bucket_duplicate_records(records: list[dict], group_id: str) -> list[list[dict]]:
    """Partition same-name duplicates into safe merge buckets.

    Typed lanes now add ontology-specific labels to Entity nodes. Generic-only
    nodes can merge into a typed bucket, but buckets with disjoint typed labels
    stay separate to avoid collapsing same-name, different-type entities.
    """
    ...
    for record in ordered:
        ...
        for cluster in clusters:
            cluster_labels = cluster['typed_labels']
            assert isinstance(cluster_labels, frozenset)
            if _label_sets_compatible(cluster_labels, typed_labels):
                cluster['records'].append(record)
                cluster['typed_labels'] = frozenset(set(cluster_labels) | set(typed_labels))
                placed = True
                break
        if not placed:
            clusters.append({'typed_labels': typed_labels, 'records': [record]})

    return [cluster['records'] for cluster in clusters if len(cluster['records']) > 1]
...
async def _merge_bucket(client, group_id: str, bucket: dict) -> int:
    ...
    WHERE NOT target.uuid IN $merged_uuids
    ...
    WHERE NOT source.uuid IN $merged_uuids
    ...
    q_delete = """
    MATCH (loser:Entity)
    WHERE loser.uuid IN $loser_uuids
    DETACH DELETE loser
    """
    ...
    merge_queries.extend(
        [
            ...
            (q_delete, {'loser_uuids': loser_uuids}),
        ]
    )
...
async def dedupe_nodes(backend, host, port, group_id, dry_run=False):
    ...
    try:
        ...
        for bucket in buckets:
            ...
            total_merged += await _merge_bucket(client, group_id, bucket)

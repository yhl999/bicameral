...
2. `OPEN | REOPENED → MONITORING` — if a `Judgment` node with an `ADDRESSES` edge exists
   (or fresh `ADDRESSES` since last reopen). Convergence matches both legacy `:Judgment`
   nodes and OM-native `:OMNode {node_type:'Judgment'}` writers.
...
These convergence-written relations now carry OM-native retrieval fields (`group_id`,
`relation_id`, `relation_root_id`, `valid_at`, `lineage_parent_relation_id`, endpoint
root ids, and explicit lifecycle metadata) so `history/current/all` consumers can see
and classify them without inference. On reopen / non-closed transitions, active
`RESOLVES` relations are explicitly invalidated rather than merely disappearing by
topology guesswork.
